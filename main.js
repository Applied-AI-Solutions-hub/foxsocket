...
const localSetup=require('./local-main.cjs')(()=>win,model=>{
 providerStore.save(providersFile(),{active:'local',models:{local:model}});hostedVerified=false;
...
ipcMain.handle('gateway',async()=>{const cfg=providerStore.load(providersFile());const probe=cfg.active==='local'?await localSetup.status(cfg.models.local||llm.getProvider('local').defaultModel):await llm.probeProvider({providerId:cfg.active,apiKey:cfg.keys[cfg.active],model:cfg.models[cfg.active]});return{ok:probe.ok===true&&(cfg.active==='local'||hostedVerified),discord:'unused',checked:Date.now(),provider:cfg.active,model:cfg.models[cfg.active]||llm.getProvider(cfg.active)?.defaultModel,error:probe.error};});
...
ipcMain.handle('providers-test',async()=>{const cfg=providerStore.load(providersFile());if(cfg.active==='local'){const model=cfg.models.local||llm.getProvider('local').defaultModel;const result=await localSetup.verify(model);return {ok:result.verified,providerId:'local',model,error:result.error,reply:result.reply};}const result=await llm.probeProvider({providerId:cfg.active,apiKey:cfg.keys[cfg.active],model:cfg.models[cfg.active]});return {...result,reachable:result.ok,ok:result.ok&&hostedVerified,error:result.ok&&!hostedVerified?'Account access checked. Send a message to verify the selected model.':result.error};});
