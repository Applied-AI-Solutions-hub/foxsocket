...
// Sparky Reflex (github.com/Applied-AI-Solutions-hub/sparky-reflex): optional local routing. Chat falls back to the model when it is down.
const reflex=require('./reflex-client.cjs').createReflex({instructions:'Which Foxsocket feature should handle this message?',routes:{email:'reading, summarizing, triaging or replying to email',tasks:'adding, completing or listing to-do items',lighting:'RGB lights, fan LEDs, colors, brightness',system:'PC health, CPU, GPU, memory, disk, restarts',chat:'general conversation, questions, writing or research'}});
const REFLEX_PAGES={tasks:'Tasks',lighting:'Devices',system:'System'};
...
ipcMain.handle('gateway',async()=>{const cfg=providerStore.load(providersFile());const probe=await llm.probeProvider({providerId:cfg.active,apiKey:cfg.keys[cfg.active],model:cfg.models[cfg.active]});return{ok:probe.ok===true,discord:'unused',checked:Date.now(),provider:cfg.active,model:cfg.models[cfg.active]||llm.getProvider(cfg.active)?.defaultModel,error:probe.error};});
ipcMain.handle('providers-get',()=>providerStore.snapshot(providersFile(),llm));
ipcMain.handle('providers-save',(_,patch)=>{providerStore.save(providersFile(),patch||{});return providerStore.snapshot(providersFile(),llm);});
ipcMain.handle('providers-test',async()=>{const cfg=providerStore.load(providersFile());return llm.probeProvider({providerId:cfg.active,apiKey:cfg.keys[cfg.active],model:cfg.models[cfg.active]});});
...
ipcMain.handle('chat',async(_,input)=>{const message=typeof input==='string'?input:input?.text,force=input?.force===true;if(hostManager.get().busy&&hostManager.get().operation==='prepare')throw Error('Host setup is still running. Wait for its checks to finish before sending a message.');if(busy)throw Error('Sparky is still responding');if(typeof message!=='string'||!message.trim()||message.length>12000)throw Error('Enter a message under 12,000 characters');busy=true;const entry={role:'user',text:message,time:Date.now()};state.chat.push(entry);persist();try{
 const decision=force?null:await reflex.route(message);
 if(decision){entry.route=decision;const target=REFLEX_PAGES[decision.route];if(target&&decision.confident){state.chat.push({role:'reflex',text:`This looks like a ${decision.route} request, so I kept it local instead of waiting on the model. Open ${target} to handle it.`,page:target,route:decision.route,time:Date.now()});log(`Sparky Reflex routed a message to ${target} in ${decision.ms} ms.`);return state;}}
 const cfg=providerStore.load(providersFile());const history=state.chat.filter(m=>m.role==='user'||m.role==='assistant').slice(-24).map(m=>({role:m.role,content:m.text}));const result=await llm.completeChat({providerId:cfg.active,model:cfg.models[cfg.active]||undefined,apiKey:cfg.keys[cfg.active],maxTokens:2048,messages:[{role:'system',content:agentFiles.systemPrompt(agentRoot())},...history]});if(!result.content)throw Error('Sparky returned no text. Check your model settings and try again.');const absorbed=agentFiles.absorbReply(agentRoot(),result.content,{summary:(message.slice(0,120)+' → '+result.content.slice(0,200))});state.chat.push({role:'assistant',text:absorbed.visible,time:Date.now(),model:result.model,provider:result.providerId});log('Sparky replied with '+((result.providerId||cfg.active)+' / '+(result.model||'model'))+'.');return state;}catch(e){state.chat.push({role:'error',text:'Reply could not be confirmed. '+e.message,time:Date.now()});persist();throw e;}finally{busy=false;}});



ipcMain.handle('reflex-status',()=>reflex.status());
