(() => {
 ...
 window.localSetupView=({local,choice,models,metrics})=>{
  ...
  return `<div class="page-intro"><h1>${ready?'Your local model is ready':'Run Sparky on this PC'}</h1><p>Choose a model. Foxsocket installs Ollama, downloads the model, and checks basic arithmetic and instruction following through the chat prompt. No API key is required.</p></div>
   ...
   <section class="panel" id="local-setup-progress" tabindex="-1" aria-labelledby="local-progress-title"><h2 id="local-progress-title">2. Setup progress</h2><strong role="status">${esc(local.message||'Ready to start.')}</strong><p>Ollama may open its own welcome window after installation. No sign-in or setup there is needed for Foxsocket; return here to follow progress.</p><ol>${stages.map(([,label])=>`<li>${label}</li>`).join('')}</ol>
   ...
   ${local.detail?`<details><summary>Download details</summary><p>${esc(local.detail)}</p></details>`:''}
