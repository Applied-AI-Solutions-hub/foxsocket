(() => {
 ...
 window.localSetupView=({local,choice,models,metrics})=>{
  ...
  const stages=[['downloading-runtime','Ollama installation'],['downloading-model','Model download'],['verifying','First reply']];
  const known=models.some(m=>m.id===choice);
  return `<div class="page-intro"><h1>${ready?'Your local model is ready':'Run Sparky on this PC'}</h1><p>Choose a model. Foxsocket installs Ollama, downloads the model, and checks a real reply. No API key is required.</p></div>
   ...
   <label class="field">Model<select id="local-model" ${busy?'disabled':''}>${models.map(m=>`<option value="${esc(m.id)}" ${m.id===choice?'selected':''}>${esc(m.label)} — ${esc(m.download)}</option>`).join('')}${!known?`<option selected value="${esc(choice)}">${esc(choice)} (custom)</option>`:''}</select></label>
   ...
   <section class="panel" aria-live="polite"><h2>2. Setup progress</h2><ol>${stages.map(([,label])=>`<li>${label}</li>`).join('')}</ol><strong>${esc(local.message||'Ready to start.')}</strong>
   ...
   ${local.reply?`<p>Last verification reply: <q>${esc(local.reply)}</q></p>`:''}
   <p>${busy?'Keep Foxsocket open during setup. If you exit or restart, return here to resume.':'Your selected model and last setup result are saved on this PC.'}</p>
