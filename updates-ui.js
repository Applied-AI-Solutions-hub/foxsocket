(()=>{
  ...
  const descriptions={idle:'A release check runs each time you open the app.',checking:'Checking GitHub releasesâ€¦',current:'You have the newest compatible release, or a newer local build.',available:'A newer version is ready to download.', 'access-required':'GitHub rejected the release check. Try again later.','rate-limited':'GitHub has temporarily limited requests. Try again later.',unavailable:'Could not check GitHub. Your workspace is still available.','no-releases':'No compatible published Windows installers were found.'};
  function paint(){
    ...
    notice.hidden=current.status!=='available';notice.textContent=`Version ${current.latest||''} available Â· View release`;
    ...
    panel.querySelector('#release-details').textContent=`Installed: ${current.current||'â€”'} Â· ${current.channel||'Release'} channel${current.checkedAt?' Â· Checked '+new Date(current.checkedAt).toLocaleTimeString():''}`;
  ...
  document.addEventListener('click',event=>{const b=event.target.closest('[data-update]');if(!b)return;const action=b.dataset.update;if(action==='check')update('updates-check');if(action==='open')window.desktop.invoke('updates-open');});
