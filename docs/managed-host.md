...
The diagnosis and repair logic above (`host-manager.js`, `host-main.js`) is implemented and unit-tested, but **not yet wired into any UI**. There is no renderer panel showing doctor findings or a runtime-failure's classified reason, and no button that calls `host-repair`. It is reachable only from `preload.js`'s allowlist and `ipcMain`, not from anything a tester can click.

`openclaw doctor --json`'s output shape was captured against a real, running install (Ubuntu-24.04, OpenClaw 2026.9.3) on 2026-09-21, not just inferred from documentation: `{ok, checksRun, checksSkipped, findings}`, with each finding carrying `checkId`, `severity`, `message`, and usually `path`/`requirement`/`fixHint`, plus an optional `target` on per-agent checks. Confirmed on that same real install: the command exits `0` even when `ok:false` with multiple findings present, matching its documented "read-only, exits successfully after producing a report" behavior — this matters because `probe()`'s parsing is gated on the process exit code. `host-manager.test.cjs`'s "doctor output shape matches a real openclaw doctor --json payload" test mirrors this real shape with scrubbed placeholder values (the actual captured findings named this machine's real MCP servers, agent names, and a plaintext-API-key config path, which don't belong in a public repo's test fixtures). `--fix`'s behavior has not been run for real yet — only `--json` has.
...
`host-manager.test.cjs` (16 cases as of this patch) covers the diagnosis/repair logic against a mocked `execute`: doctor findings attach without blocking readiness, `repair()` calls `openclaw doctor --fix --non-interactive` and re-verifies, a failed repair reports cleanly, runtime-install failures are classified by mocked exit codes (6/7/28/35/60/23, plus an unrecognized code falling back to the generic message), and the two systemd sub-cases produce distinct guidance. The `--json` shape has been verified against a real install (see "Not complete" above); `--fix` has not been run for real yet.
