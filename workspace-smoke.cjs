...
require('./main');
let chatCalls=0;
for(const action of ['gateway','metrics','setup-check','chat'])ipcMain.removeHandler(action);
...
app.whenReady().then(async()=>{
 ...
 try{
  ...
  const result=await win.webContents.executeJavaScript(`(async()=>{
   ...
   $('[data-role=host]').click();await wait(100);check($('#content').textContent.includes('None found'),'Fresh PC requirements');check($('#content').textContent.includes('Ubuntu needs Windows setup first'),'Missing Linux guidance');
   ...
   $('[data-page=devices]').click();check(!$('.lighting-panel').open,'Lighting starts collapsed');$('.lighting-panel').open=true;await wait(20);$('[data-action=lighting]').click();await wait(200);check($('.lighting-panel').open,'Lighting stays open after scan');check(!$('#content').textContent.includes('Skytech'),'No fixed test hardware');
   $('.lighting-panel').open=false;await wait(20);$('[data-action=metrics]').click();await wait(60);check(!$('.lighting-panel').open,'Lighting stays collapsed after refresh');
   ...
   $('[data-page=chat]').click();return {freshHostSetup:true,savedRole:true,offlineHelp:true,draftRetention:true,duplicateGuard:true,formattedReply:true,escapedHTML:true,failureRecovery:true,tasks:true,lightingDisclosure:true,genericDevices:true,closePreference:true,reducedMotion:true};
  })()`);
